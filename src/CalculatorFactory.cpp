...
std::unique_ptr<Calculator> CalculatorFactory::createCalculator()
{
    ...
    return std::make_unique<Calculator>(
        window, std::make_unique<TilesLayout>(), std::make_unique<calculator::UserInputReader>(window),
        buffer, std::make_unique<calculator::InteractionHandler>(),
        std::make_unique<calculator::IntersectedTileInformationInterpreter>(buffer));
}
}